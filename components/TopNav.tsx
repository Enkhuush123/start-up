...
export default function TopNav() {
    ...
    return (
        <>
        <nav className="fixed top-0 w-full h-20 bg-neutral-950/80 backdrop-blur-2xl border-b border-neutral-800 z-50 flex items-center justify-between px-4 md:px-10 transition-colors">
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.name}
                            href={item.href}
                            className={`relative flex items-center gap-2 text-sm font-bold transition-all ${isActive ? "text-pink-500 drop-shadow-[0_0_10px_rgba(236,72,153,0.5)]" : "text-neutral-400 hover:text-white"
                                }`}
                        >
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                <button onClick={toggleLang} className="flex items-center gap-1 text-xs font-bold text-neutral-400 hover:text-white bg-neutral-900 px-3 py-2 rounded-full transition-all border border-neutral-800">
                    <Globe size={14} /> {lang}
                </button>
                
                <button onClick={toggleTheme} className="text-neutral-400 hover:text-white bg-neutral-900 p-2.5 rounded-full transition-all border border-neutral-800">
                    {isDark ? <Sun size={16} /> : <Moon size={16} />}
                </button>

                {session ? (
                    <Link href="/profile" className="ml-2 w-10 h-10 rounded-full bg-neutral-800 border-2 border-neutral-700 hover:border-pink-500 flex items-center justify-center overflow-hidden transition-all shadow-lg">
                        <User size={18} className="text-neutral-300" />
                    </Link>
                ) : (
                    <>
                        <Link href="/login" className="hidden sm:block text-sm font-bold text-neutral-300 hover:text-white transition-colors ml-2">
                            Нэвтрэх
                        </Link>
                        <Link href="/login">
        ...
        {}
        {!isAuthPage && session && (
            <div className="md:hidden fixed bottom-0 left-0 w-full h-16 bg-neutral-950/90 backdrop-blur-2xl border-t border-neutral-800 z-50 flex items-center justify-around px-2">
                {navItems.map((item) => {
                    const isActive = pathname === item.href;
                    const Icon = item.icon;
                    return (
                        <Link
                            key={item.name}
                            href={item.href}
                            className={`relative flex flex-col items-center justify-center w-16 h-full transition-all ${isActive ? "text-pink-500" : "text-neutral-500 hover:text-white"
                                }`}
                        >
